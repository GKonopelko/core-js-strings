...
function removeFirstOccurrences(str, value) {
  return str.indexOf('value') ? str.replace(value, '') : 0;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (str.indexOf(value) >= 0) {
    return str.substring(0, i1) + str.substring(i2, str.length);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  ...
  const min = minutes.toString(10).padStart(2, 0);
  const sec = seconds.toString(10).padStart(2, 0);
  return `${min}:${sec}`;
}
...
function isPalindrome(str) {
  const low = str.replace(/[^a-zA-Z0-9'']/g, '').toLowerCase();
  return low.split('').reverse().join('') === low;
}
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').sort(function sort(a, b) {
